    ...
    if @object.horizontal_code_list.codes.length > 1
      json.partial! 'response_domains/show',
                    rd: @object.rds_qs.find_by_code_id(x.value).nil? ?
                        nil : @object.rds_qs.find_by_code_id(x.value).try(:response_domain)
    else
      json.partial! 'response_domains/show', rd: @object.rds_qs.first.try(:response_domain)
    end
  end
end
...
json.pretty_corner_label @object.pretty_corner_label
